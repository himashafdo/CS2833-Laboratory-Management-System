...
public ResponseEntity<?> createInternalTicket(@RequestBody Map<String, Object> body,
    ...
    try {
        ...
        return ResponseEntity.ok(ticketService.createInternalTicket(title, description, priority, equipmentId, token));
    } catch (Exception e) {
        return ResponseEntity.badRequest().body(Map.of("error", e.getMessage()));
    }
}
